...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        bees = []
        range = 0
        max_range = self.max_range if hasattr(self, "max_range") else type(self).max_range
        min_range = self.min_range if hasattr(self, "min_range") else type(self).min_range
        while range <= max_range and current_place is not beehive and not bees:
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        if self.armor - amount <= 0:
            attack += FireAnt.damage if not hasattr(self, "damage") else self.damage
        for bee in list(self.place.bees):
            bee.reduce_armor(attack)
...
class NinjaAnt(Ant):
    ...
    def action(self, gamestate):
        # BEGIN Problem 7
        "*** YOUR CODE HERE ***"
        damage = NinjaAnt.damage if not hasattr(self, "damage") else self.damage
        for bee in list(self.place.bees):
...
class TankAnt(ContainerAnt):
    ...
    def action(self, gamestate):
        ...
        damage = TankAnt.damage if not hasattr(self, "damage") else self.damage
        for bee in list(self.place.bees):
             bee.reduce_armor(damage)
        # END Problem 10
...
class Water(Place):
    ...
    def add_insect(self, insect):
        ...
        is_watersafe = insect.is_watersafe if hasattr(insect, "is_watersafe") else type(insect).is_watersafe
        if not is_watersafe:
...
# END Problem 13
    ...
    def action(self, gamestate):
        ...
        def double_damage(ant):
            if not hasattr(ant, "damage"):
                ant.damage = type(ant).damage * 2
            else:
                ant.damage *= 2
        ...
        if self.is_queen:
            ...
            while current_place != None:
                ...
                if ant is not None and ant not in QueenAnt.already_doubled:
                    double_damage(ant)
                    QueenAnt.already_doubled.add(ant)
...
class Bee(Insect):
    ...
    def blocked(self):
        ...
        if self.place.ant is not None:
            if hasattr(self.place.ant, "blocks_path"):
                return self.place.ant.blocks_path
            return type(self.place.ant).blocks_path
        return False
        # END Problem 7
    ...
    def action(self, gamestate):
        ...
        # BEGIN EC
        "*** YOUR CODE HERE ***"
        if not self.direction and self.place.entrance != None:
...
def apply_status(status, bee, length):
    ...
    status_action = status(current_action, bee)
    
    def repeat_status_action(gamestate):
        nonlocal length
        if length <= 0:
            current_action(gamestate)
            bee.action = current_action
        else:
            status_action(gamestate)
        length = length - 1
    
    bee.action = repeat_status_action
    # END Problem EC
