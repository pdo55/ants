...
class Place(object):
    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            # BEGIN Problem 13
            "*** YOUR CODE HERE ***"
            if not isinstance(insect, QueenAnt) or insect.number > 1:
            # END Problem 13
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        distance = 0
        cont = True
        while cont and nearest_place.entrance != hive:
            if nearest_place.bees and distance >= self.min_range and distance <= self.max_range:
                cont = False
            else:
...
# END Problem 13
    ...
    def __init__(self, armor=1):
        ...
        QueenAnt.queen_ants_created += 1
        self.number = QueenAnt.queen_ants_created
        self.ants_buffed = []
        # END Problem 13
    ...
    def action(self, colony):
        ...
        if self.number > 1:
            ...
            while behind_place:
                if behind_place.ant:
                    if behind_place.ant.is_container and behind_place.ant.contained_ant:
                        if not behind_place.ant.contained_ant in self.ants_buffed:
                            behind_place.ant.contained_ant.damage *= 2
                            self.ants_buffed.append(behind_place.ant.contained_ant)
                    if not behind_place.ant in self.ants_buffed:
                        behind_place.ant.damage *= 2
                        self.ants_buffed.append(behind_place.ant)
    ...
    def reduce_armor(self, amount):
        ...
        if self.armor <= 0 and self.number == 1:
            bees_win()
        # END Problem 13
...
def apply_effect(effect, bee, duration):
    ...
    # time_passed = []
    turns = 0
    def effecting(colony):
        # time_passed.append(colony.time)
        # if time_passed[-1] - time_passed[0] <= duration:
        nonlocal turns
        turns += 1
        if turns <= duration:
            return effect(original_action, bee)(colony)
        bee.forward = True
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self, hive):
        ...
        front = self.place
        distance = 0
        while front is not hive:
            if front.ant and front.ant is not self:
                insects_dict[front.ant] = distance
